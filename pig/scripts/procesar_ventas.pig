...
-- Aquí usamos FLATTEN para aplanar el 'bag' y luego calculamos el promedio
estadisticas = FOREACH por_categoria {
    flattened = FLATTEN(ventas);
    GENERATE group AS categoria, 
    COUNT(flattened) AS count, 
    AVG((double)flattened.monto) AS avg_monto;
}
